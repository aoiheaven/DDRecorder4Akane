...
def get_manual_cut_points(danmu_list: List[Dict], uid: List[str], cmdheader_list: List[str]) -> List[Tuple[datetime.datetime, datetime.datetime, List[str]]]:
    cut_points = []
    count = 0
    for danmu_obj in danmu_list:
        try:
            ifClipHeader = any([danmu_obj['text'].startswith(cmdheader) for cmdheader in cmdheader_list])
            if danmu_obj['uid'] in uid and ifClipHeader:
                count += 1
    ...
    def run(self) -> bool:
        try:
            ...
            if self.config.get('spec', {}).get('manual_clipper', {}).get('enabled', False):
                danmu_list = parse_danmu(self.danmu_path)
                cmdheader_list = self.config.get('spec', {}).get('manual_clipper', {}).get('cmdheader', [])
                cut_points = get_manual_cut_points(danmu_list, self.config.get(
                    'spec', {}).get('manual_clipper', {}).get('uid', []), cmdheader_list)
...
if __name__ == "__main__":
    ...
    danmu_list = parse_danmu("data/danmu/411318_2022-04-24_21-27-05")
    # counted_danmu_dict = count(
